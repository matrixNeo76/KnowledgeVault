...
export const ResourceModal: React.FC<ResourceModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div 
        className="bg-[#0D0D0D] border border-[#222] rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-150"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Top Bar */}
        <div className="p-5 border-b border-[#1C1C1C] flex items-center justify-between bg-[#0A0A0A]">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-md bg-[#161616] border border-[#262626] text-[#C5A059] text-xs font-mono font-medium">
              {getTypeIcon(resource.type)}
              <span className="capitalize">{resource.type === "knowledge" ? "OKF v0.2 Knowledge" : resource.type.replace("_", " ")}</span>
            </span>

            {resource.createdAt && (
              <span className="hidden sm:flex items-center gap-1 text-[11px] font-mono text-[#666]">
          ...
          <div className="flex items-center gap-2">
            {!isEditing ? (
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-[#888] hover:text-white bg-[#141414] hover:bg-[#1F1F1F] border border-[#262626] px-3 py-1.5 rounded-lg transition-colors"
              >
              ...
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="flex items-center gap-1.5 text-xs text-black bg-[#C5A059] hover:bg-[#D5B069] font-medium px-3 py-1.5 rounded-lg transition-colors"
              >
            ...
            <button
              ...
              disabled={isDeleting}
              className="p-1.5 text-[#666] hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors"
            ...
            <button
              onClick={onClose}
              className="p-1.5 text-[#666] hover:text-white hover:bg-[#1C1C1C] rounded-lg transition-colors ml-2"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        {/* Modal Body */}
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-[#CCC]">
          {isEditing ? (
            /* Editing Form */
